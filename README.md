...
原项目把「暴躁教授」人设与 RAG 读论文结合，已经很好玩。我们在不推翻架构的前提下，从**阅读体验、配置安全、人设趣味**等方向做了增强，主要包括：

1. **可点击的引用胶囊（Citation Pill）**  
   AI 回答里若出现基于检索的引用标记（如 `*ref-[N]`），会渲染成可点击的小标签；点击后左侧正文自动滚动到对应段落。比「自己全文找」省时间，也方便核对 AI 是不是在瞎扯——教授再暴躁，也得指哪儿打哪儿。

2. **导师风格一键切换**  
   聊天区标题旁提供导师风格菜单，自动扫描 `prompt/` 下所有 `ai_character_prompt_*.txt`（例如可莉、雷电等），无需改代码路径即可切换人设。适合演示时瞬间换「导师人格」，增加节目效果。

3. **非标准论文 / 幻灯片式 PDF 的兼容入口**  
   原版更偏标准学术论文结构。我们增加 `MarkdownProcessorSlides`（幻灯片处理器），并在顶部设置里提供「使用幻灯片处理器」开关，让版式不像经典论文的 PDF 也有机会被结构化解析、走后续管线。（复杂版式仍可能翻车，但显著拓宽了「能试着读一读」的文档类型。）

4. **API 配置外部化 + 图形设置**  
   密钥与 Base URL 从硬编码改为读写项目目录下的 `app_settings.json`（该文件已加入 `.gitignore`，避免误提交）。应用内 ⚙ → **API 设置…** 可填写 DeepSeek / MiniMax 等配置，保存后自动刷新 LLM 客户端。同时 `.gitignore` 忽略 `data/`、`output/` 等运行时目录，仓库更干净。

## 更改日志:

2026/4/7

- **可点击引用胶囊**：检索引用标记 `*ref-[N]` 渲染为可点击标签，点击后左侧正文滚动到对应段落
- **导师风格菜单**：自动发现 `prompt/ai_character_prompt_*.txt`，标题旁一键切换人设，无需改代码中的 prompt 路径
- **幻灯片处理器**：新增 `MarkdownProcessorSlides`，设置中「使用幻灯片处理器」开关，兼容非标准论文 / 幻灯片式 PDF 管线
- **API 与仓库卫生**：`app_settings.json` 外部化密钥与 Base URL，图形界面配置并刷新客户端；`.gitignore` 忽略 `app_settings.json`、`data/`、`output/` 等
